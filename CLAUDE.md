...
- [x] Visual playtest pass — "looks better" after the heightmap-envelope tweak (`dd456ca`) and elevation-aware BiomeSource fix (`69721ca`). User flagged some residual issues but elected to defer; tracked as M3 backlog below.
...
## M3 backlog (deferred polish, address before M6+)

- **Coastal smear (5-10 block strip of "wrong" biome at coastlines).** BiomeSource uses the *un-eroded* noise heightmap for the ocean/land split; erosion can shift actual surface by 5-10 blocks. Fix options: (a) sample the eroded RegionCache from BiomeSource (requires plumbing); (b) add a "coastal" biome bucket with appropriate vanilla biomes; (c) carve a small "beach" margin via surface rules.
- **Climate seed is hardcoded `0`.** Climate map is identical across worlds — only the terrain heightmap varies with seed. Fix: derive a stable climate seed from the chunk gen's `currentWorldSeed` once it's set, or seed-shift climate by the world seed at first sample.
- **Region-boundary erosion seams.** Each 256×256 region is eroded independently. Droplets that would have crossed the boundary just terminate, so the eroded surface can have a faint grid line every 256 blocks. Fix: pad each region by ~16 blocks of ghost cells (sample from neighbors), run erosion on the padded region, return inner 256×256.
- **Possible "ocean above sea level" residual cases.** User reported it's *much* better but "some issues" remain. Possibly the ones above (coastal smear) or could be edge cases where ocean biome cells from rare ocean buckets land at >sea-level columns.
...
- First playtest: "looks better but mountains look like hills." Bumped BASE_HEIGHT 70→90, AMPLITUDE 30→60, FREQUENCY 0.005→0.003 in `dd456ca`. Range now y=30..150.
- Second playtest: "oceans are spawning above water level." BiomeSource was elevation-blind at M2 by design; M3 was the right time to wire elevation. Made TerraScribeChunkGenerator publish its world seed as a static singleton, override createBiomes to set it before first biome lookup, split BiomeSource pools into oceans + climate-bucketed land. Discovery now logs "44 land biomes across 9 climate buckets, 9 ocean biomes". `69721ca`.
- Third playtest: "looks better, some issues, keep moving." → M3 closed. Issues tracked in `## M3 backlog` above.
